...
fn main() {
    let mut schoolTest : School = School::new();
    schoolTest.add(7, "Hieu");
    schoolTest.add(8, "Minh");
    schoolTest.add(9, "Huynh");
    schoolTest.add(10, "Hau");
    schoolTest.add(7, "Hai");
    schoolTest.add(6, "Trao");
    schoolTest.add(6, "Anh");

    print!("{:?}",schoolTest.grade(6) );
}
...
impl School {
    pub fn new() -> School {
         School { student: HashMap::new() }
    }

    pub fn add(&mut self, grade: u32, student: &str) {
        self.student.insert(String::from(student), grade);
    }

    pub fn grades(&self) -> Vec<u32> {
      return self.student.values().cloned().collect::<Vec<u32>>();
    }


    pub fn grade(&self, grade: u32) -> Vec<String> {
        let mut result = self.student
        .iter().filter_map(| (key, value)| if *value == grade  {Some(key.clone())} else {None})
        .collect::<Vec<String>>();
        result.sort();
        return result;
